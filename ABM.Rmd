...
dyads_viz <- dyads %>% 
  filter(ticks_tog != 0) %>% 
  select(agent_1, agent_2, ticks_tog)

tictog <- graph_from_data_frame(dyads_viz, directed = FALSE)

coords <- layout.fruchterman.reingold(tictog)
