...
Cl's are updated at end of tick - meaning the cl noted next to an outcome is not the cl used for math - the cl of the previous row is.
...
cl_alt_b <- as.numeric(rep(cl_alt_setup, each=N))

cl_sat_b <- sample(1:10, N, replace = TRUE)
cl_sat_b <- as.numeric(rep(cl_sat_setup, each=N))
...
filename <- sprintf("logfiles/traits_%s_ticks_%s_agents_%s_%s.csv", ticks, N, sort, invite_function)
traits  %>% write_csv(filename)
