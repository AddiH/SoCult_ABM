...
commit = commit + (inv * ticks_spent_tog) (evt. cap)

 -   There is no penalty for not cooperating  - das ok. np.
 
 -   No joint control - we can argue that random intercept will make each situation different

 -   Agents have no reason to prioritize more than one person - other
     than randomish invites - das ok
     
 -   What dist to draw from when making random intercepts? 
 
sample from see paper
...
Add zero protection to generate.matrix : if dep 0 =ac + pc = 0

Viz: barplot pr. dyad med sat, dep og commit udvikling over tid
